...
## changer A parcours de fichier sur 10%
# Parse file
...
def gfftobed(gff,AugProt_bed):# conversion en fichier bed
    if os.path.exists(auggff+'/'+gff+'.gff'):
        str1 = " awk -F'\t' '$3~/^gene/' "+auggff+'/'+gff+".gff | awk -F'\t' '{print $1,$4,$5,$6,$7,$8,$9,$2,$3}' OFS='\t' > "+AugProt_bed+'/'+gff+".bed"
        content = os.popen(str1).read()

## essayer avec bedtools parce que fichiers manquants
def gfftofasta(gff,augProt_faa): # extraction seq prot
    if os.path.exists(auggff+'/'+gff+'.gff'):
        wd=os.getcwd()
        str1 = "perl "+wd+"/getAnnoFasta.pl "+auggff+'/'+gff+".gff ; mv "+auggff+'/'+gff+".aa "+augProt_faa+'/.'
        content = os.popen(str1).read()

def addH(f,i): # add metagid in fasta header
    str1 = "./addH.sh "+f+'/'+i+'.aa '+i
    content = os.popen(str1).read()

def Concat(r):
    str2 = "cat "+r+"/*_hid > "+r+'.aa' ##add name
...
def diamond(fileInPath): ## ajouter singularity avec variable sing et sing2
    str1 = " diamond blastp -p 60 --more-sensitive  -e 0.000001 --outfmt 102 -d /work/cbelliardo/NR_2020_01_diamond.dmnd -q "+ fileInPath+".aa -o " + fileInPath+".dmd"
    content = os.popen(str1).read()
